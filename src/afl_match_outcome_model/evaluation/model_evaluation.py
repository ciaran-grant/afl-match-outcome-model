...
class ClassifierEvaluator(ModelEvaluator):
    ...
    def get_accuracy(self):
        """Return the accuracy."""
        print("Expected Accuracy: \t{:.4f}".format(accuracy_score(self.actual, self.expected_label)))
        if self.compare_name is not None:
            print("Compare Accuracy: \t{:.4f}".format(accuracy_score(self.actual, self.compare_label)))
            return accuracy_score(self.actual, self.expected_label), accuracy_score(self.actual, self.compare_label)
        return accuracy_score(self.actual, self.expected_label)
        

    def get_recall(self):
        """Return the recall."""
        print("Expected Recall: \t{:.4f}".format(recall_score(self.actual, self.expected_label)))
        if self.compare_name is not None:
            print("Compare Recall: \t{:.4f}".format(recall_score(self.actual, self.compare_label)))
            return recall_score(self.actual, self.expected_label), recall_score(self.actual, self.compare_label)
        return recall_score(self.actual, self.expected_label)
    
    def get_precision(self):
        """Return the precision."""
        print("Expected Precision: \t{:.4f}".format(precision_score(self.actual, self.expected_label)))
        if self.compare_name is not None:
            print("Compare Precision: \t{:.4f}".format(precision_score(self.actual, self.compare_label)))
            return precision_score(self.actual, self.expected_label), precision_score(self.actual, self.compare_label)      
        return precision_score(self.actual, self.expected_label)
          
    def get_f1_score(self):
        """Return the f1 score."""
        print("Expected F1 Score: \t{:.4f}".format(f1_score(self.actual, self.expected_label)))
        if self.compare_name is not None:
            print("Compare F1 Score: \t{:.4f}".format(f1_score(self.actual, self.compare_label)))
            return f1_score(self.actual, self.expected_label), f1_score(self.actual, self.compare_label)      
        return f1_score(self.actual, self.expected_label)
